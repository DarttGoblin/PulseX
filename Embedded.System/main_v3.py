...
# === Check user metadata ===
def check_device_enabled():
    try:
        with open("device_state.json", "r") as f:
            state = json.load(f)
        return state.get("device_enabled", False), state.get("metadata", {})
    except:
        return False, {}

print("🛑 Waiting for user metadata...")
metadata_ready = False
metadata = {}

while not metadata_ready:
    enabled, metadata = check_device_enabled()
    if enabled:
        metadata_ready = True
        break
    time.sleep(2)

print("✅ Metadata received, device is now enabled!")
print("User metadata:", metadata)

def get_demo_tensor():
    return torch.tensor([[metadata.get("age",22)/100.0,
                          metadata.get("gender",1.0),
                          metadata.get("region",0.0),
                          metadata.get("smoker",1.0)]], dtype=torch.float).to(device)
